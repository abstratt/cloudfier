...
public class KirraMDDRuntimeExternalServiceTests extends AbstractKirraMDDRuntimeTests {
	...
	protected Properties createDefaultSettings() {
		...
		creationSettings.setProperty(KirraMDDConstants.EXTERNAL_CONNECTOR_URI, "http://localhost:" + TEST_SERVER_PORT + "/externalpath/");
	...
	public void testRetriever() throws Exception {
        ...
        delegate = new AbstractHandler() {
        	...
        	public void handle(String target, Request baseRequest,
                ...
                BigInteger result = new BigInteger(baseRequest.getParameter("number1")).add(new BigInteger(baseRequest.getParameter("number2")));
		...
		executeKirraOperation(created.getEntityNamespace(), created.getEntityName(), created.getObjectId(), "compute", Collections.emptyList());
		assertNotNull(requestPath[0]);
		assertEquals("&number1=31&number2=11", requestQuery[0]);
		assertEquals("GET", requestMethod[0]);
		assertEquals("/externalpath/retrievers/tests.CalculatorService/addNumbers", requestPath[0]);

		Instance reloaded = kirra.getInstance("tests", "Sum", created.getObjectId(), true);
		assertEquals(42L, reloaded.getValue("result"));
	}
	...
	public void testEvent() throws Exception {
		...
		assertNotNull(requestPath[0]);
		assertEquals("&", requestQuery[0]);
		assertEquals("POST", requestMethod[0]);
		assertEquals("/externalpath/events/tests.EmailService/issueChanged", requestPath[0]);
		
		assertNotNull(requestEntity[0]);
		ObjectNode parsedEntity = JsonHelper.parse(new StringReader(requestEntity[0]));
		assertEquals(Arrays.asList("typeName", "values"), IteratorUtils.toList(parsedEntity.getFieldNames()));
		assertEquals(new TextNode("tests.IssueChange"), parsedEntity.get("typeName"));
		...
		assertEquals(Arrays.asList("issueNumber", "newStatus"), fieldNames);
	}
