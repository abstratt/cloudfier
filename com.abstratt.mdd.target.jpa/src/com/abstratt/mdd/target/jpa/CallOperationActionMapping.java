...
public class CallOperationActionMapping implements IActionMapper<CallOperationAction> {
	...
	private static final Map<String, String> OPERATOR_MAPPING = asMap(new String[][] {
			{ "greaterOrEquals", ">=" }, { "lowerThan", "<" },
			{ "equals", "=" }, { "lowerOrEquals", "<=" },
			{ "greaterThan", ">" }, { "add", "+" }, { "subtract", "-" }, { "divide", "/" }, { "multiply", "*" }, {"same", "is"}  });
	...
	public String map(CallOperationAction coa, IMappingContext context) {
		...
		if (arguments.size() == 1) {
			result.append(ActionMappingUtils.mapSourceAction(coa.getTarget(), context));
			result.append(" ");
			result.append(sqlOperator);
			result.append(" ");
			result.append(ActionMappingUtils.mapSourceAction(arguments.get(0), context));
