...
public class GeneratorTests extends AbstractWebFrontEndTest {
	...
	public void testMissingPlatform() throws HttpException, IOException {
		...
		source += "end.\n";
		buildProject(source);
		GetMethod generateRequest = new GetMethod(getTestGeneratorURI().resolve(
				"non-existing").toASCIIString());
		executeMethod(404, generateRequest);
	}

	private URI getTestGeneratorURI() {
		return URI.create("http://localhost"
		+ WebFrontEnd.GENERATOR_PATH + getName() + "/");
	}

	public void testMissingWorkspace() throws HttpException, IOException {
		GetMethod generateRequest = new GetMethod(getTestGeneratorURI().resolve("pojo")
				.toASCIIString());
		executeMethod(404, generateRequest);
	}
	...
	public void testCustomPlatform() throws HttpException, IOException {
		...
		Assert.assertTrue(generated,
				AssertHelper.areEqual("hello, Bar", generated));
	}
...
//	public void testGenerateCustomMultiple() throws HttpException, IOException {
//		String source = "";
//		source += "package foo;\n";
//		source += "class Class1\n";
//		source += "end;\n";
...
//	}
	...
	private byte[] generate(String platform, String expectedType,
			String... classes) throws HttpException, IOException {
		GetMethod generateRequest = new GetMethod(getTestGeneratorURI().resolve(
				platform).toASCIIString());
