package com.abstratt.mdd.modelimporter.jdbc
...
class JDBCImporter {
	...
	new(Properties properties) {
		...
		this.fragments = properties.get("mdd.importer.jdbc.table.fixcase.fragments").toString().split(',').map[trim].
			toList
	...
	def Map<String, CharSequence> generateTextUMLModel(Catalog catalog) {
	...
	def static void main(String[] args) {
		...
		importer.generateTextUMLModel(catalog).forEach [ p1, p2 |
			println("key: " + p1)
			println(p2)
		]
	}
