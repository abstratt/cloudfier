...
public class StormpathAuthenticationService implements AuthenticationService {
    ...
    private class CachedAuthentication {
		...
		public boolean isExpired() {
			return (System.currentTimeMillis() - this.timestamp) > expiryPeriod;
		}
	...
	private boolean sendRequest(String path, Map<String, Object> request, int expected) {
		try {
			...
			client.getState().setCredentials(
			    new AuthScope("api.stormpath.com", 443),
			...
			LogUtils.logInfo(AuthenticationService.class.getPackage().getName(), "Status: " + status + " - " + send.getResponseBodyAsString(), null);
