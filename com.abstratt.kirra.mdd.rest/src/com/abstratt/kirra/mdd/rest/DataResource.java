...
public class DataResource extends AbstractKirraRepositoryResource {
	...
	public Representation create(Representation noneExpected) {
		...
		if (Boolean.parseBoolean(repo.getProperties().getProperty("mdd.isLibrary"))) {
			...
			return new StringRepresentation("{\"success\": false, \"message\": \"Cannot deploy database for a library project\" }", MediaType.APPLICATION_JSON);	
		}
		repo.initialize();
      	DataPopulator populator = new DataPopulator(repo);
      	boolean success = populator.populate();
      	ResourceUtils.ensure(success, "Failure populating the database, check the data sample", Status.CLIENT_ERROR_BAD_REQUEST);
