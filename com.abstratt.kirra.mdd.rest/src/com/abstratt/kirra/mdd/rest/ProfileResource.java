...
public class ProfileResource extends AbstractKirraRepositoryResource {
	...
	/**
	 * Request entity is the same as you would use to create a contact in the application.
	 */
	@Post("json")
	public Representation createProfile(Representation requestRepresentation) throws IOException {
		...
		ResourceUtils.ensure(currentUserName != null && !"guest".equalsIgnoreCase(currentUserName.trim()), "A user needs to be logged in when creating a profile", Status.CLIENT_ERROR_UNAUTHORIZED);

		RuntimeObject existingProfile = KirraActorSelector.findUserInstance(currentUserName, Runtime.get());
		ResourceUtils.ensure(existingProfile == null, "Logged in user already has a profile, delete it first", Status.CLIENT_ERROR_BAD_REQUEST);
