...
public class EntityResource extends AbstractKirraRepositoryResource {
	...
	public Representation getEntity() {
		...
		for (Relationship relationship : entity.getRelationships()) {
			...
			relationshipRepr.typeUri = entityRepresentation.uri = referenceBuilder.buildEntityReference(relatedTypeRef.getEntityNamespace(), relatedTypeRef.getTypeName()).toString(); 
		...
		for (Operation operation : entity.getOperations()) {
			...
			case Finder:
				...
				queryRepr.uri = reference.getParentRef().getParentRef().addSegment(Paths.FINDERS).addSegment(entityNamespace + '.' + entityName).addSegment(operation.getName()).toString();
			...
			case Action: 
				...
				if (!operation.isInstanceOperation()) {
					actionRepr.uri = reference.getParentRef().getParentRef().addSegment(Paths.ACTIONS).addSegment(entityNamespace + '.' + entityName).addSegment(operation.getName()).toString();
