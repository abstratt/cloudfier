...
public class RuntimeAssociationTests extends AbstractRuntimeTests {
	...
	private void testOneToMany(String behavior) throws CoreException {
		...
		// check links
		assertEquals(e1, runStaticOperation("tests::TestDriver", "getEndOne", e2a));
		assertEquals(e1, runStaticOperation("tests::TestDriver", "getEndOne", e2b));
		assertNull(runStaticOperation("tests::TestDriver", "getEndOne", e2c));
		CollectionType allMany = (CollectionType) runStaticOperation("tests::TestDriver", "getEndMany", e1);
		assertEquals(2, allMany.getBackEnd().size());
		assertTrue(allMany.contains(e2a));
		assertTrue(allMany.contains(e2b));
		assertFalse(allMany.contains(e2c));
	}
	...
	public void testReflexive() throws CoreException {
		...
		assertEquals(e2.getObjectId(), e1.getRelated(end1).iterator().next().getObjectId());
		assertEquals(e1.getObjectId(), e2.getRelated(end2).iterator().next().getObjectId());
	}
	...
	public void testOneToOneMakeLink() throws CoreException {
		...
		// check link
		assertEquals(e2, e1.getValue(end2));
		assertEquals(e1, e2.getValue(end2.getOtherEnd()));
	}
	...
	public void testOneToManyMakeLink() throws CoreException {
		...
		assertEquals(2, e1Many.size());
		assertTrue(e1Many.contains(e2a));
		assertTrue(e1Many.contains(e2b));
		assertEquals(e1a, e2a.getValue(endMany.getOtherEnd()));
		assertEquals(e1a, e2b.getValue(endMany.getOtherEnd()));
		...
		assertEquals(1, e2Many.size());
		assertTrue(e2Many.contains(e2b));
		assertEquals(e1b, e2b.getValue(endMany.getOtherEnd()));
		assertEquals(e1a, e2a.getValue(endMany.getOtherEnd()));

		assertEquals(1, ((CollectionType) e1a.getValue(endMany)).getBackEnd().size());
	}
	...
	public void testOneToOne(String behavior) throws CoreException {
		...
		// check link
		assertEquals(1, e1.getRelated(getProperty("tests::OneToOne::end2")).size());
		assertTrue(e1.getRelated(getProperty("tests::OneToOne::end2")).contains(e2));

		assertEquals(e2, runStaticOperation("tests::TestDriver", "getEnd2", e1));
		assertEquals(e1, runStaticOperation("tests::TestDriver", "getEnd1", e2));
		...
		// check link
		assertEquals(0, e1.getRelated(getProperty("tests::OneToOne::end2")).size());

		assertNull(runStaticOperation("tests::TestDriver", "getEnd2", e1));
		assertNull(runStaticOperation("tests::TestDriver", "getEnd1", e2));
	}
	...
	public void testDerivedLink() throws CoreException {
	    ...
	    assertTrue(((CollectionType) readAttribute(referrer, "numbers")).getBackEnd().containsAll(allNumbers));
	    assertEquals(2, ((CollectionType) readAttribute(referrer, "positiveNumbers")).getBackEnd().size());
	    assertTrue(((CollectionType) readAttribute(referrer, "positiveNumbers")).getBackEnd().containsAll(positiveNumbers));
	}
}
