...
public abstract class CollectionType extends BuiltInClass implements Serializable {
	...
	public static CollectionType createCollection(Type baseType, boolean unique, boolean ordered) {
		return createCollection(baseType, unique, ordered, new HashSet<BasicType>());
	}

	public static <E extends BasicType, T extends Collection<E>> CollectionType createCollection(Type baseType,
			boolean unique, boolean ordered, T backEnd) {
		assert baseType != null;
		return ordered ? (unique ? new OrderedSetType(baseType, backEnd) : new SequenceType(baseType, backEnd))
				: (unique ? new SetType(baseType, backEnd) : new BagType(baseType, backEnd));
	}
	...
	public static CollectionType createCollectionFor(MultiplicityElement element) {
		...
		return createCollection(((TypedElement) element).getType(), element.isUnique(), element.isOrdered());
	}

	public static <E extends BasicType, T extends Collection<E>> CollectionType createCollectionFor(
			MultiplicityElement element, T backEnd) {
		Assert.isTrue(element.isMultivalued());
		Assert.isTrue(element instanceof TypedElement);
		return createCollection(((TypedElement) element).getType(), element.isUnique(), element.isOrdered(), backEnd);
	}
	...
	public CollectionType add(@SuppressWarnings("unused") ExecutionContext context, BasicType toAdd) {
		CollectionType result = createCollection(baseType, isUnique(), isOrdered());
	...
	public void forEach(@SuppressWarnings("unused") ExecutionContext context, ElementReferenceType reference) {
		for (BasicType current : backEnd)
			runClosureBehavior(context, reference, current);
	}

	public BasicType reduce(@SuppressWarnings("unused") ExecutionContext context, ElementReferenceType reference,
			BasicType initial) {
		BasicType partial = initial;
		for (BasicType current : backEnd)
			partial = (BasicType) runClosureBehavior(context, reference, current, partial);
		return partial;
	}
	...
	public CollectionType select(@SuppressWarnings("unused") ExecutionContext context, ElementReferenceType reference) {
		CollectionType result = createCollection(baseType, isUnique(), isOrdered());
		for (BasicType current : backEnd) {
			BooleanType predicateOutcome = (BooleanType) runClosureBehavior(context, reference, current);
	...
	protected static Object runClosureBehavior(ExecutionContext context, ElementReferenceType reference, Object... arguments) {
		return context.getRuntime().runBehavior(context.currentFrame().getSelf(), "[closure]", (Activity) reference.getElement(), arguments);
	}
	...
	public CollectionType collect(@SuppressWarnings("unused") ExecutionContext context, ElementReferenceType reference) {
		...
		CollectionType result = createCollection(closureReturnParameter.getType(), isUnique(), isOrdered());
		for (BasicType current : backEnd) {
			BasicType mapped = (BasicType) runClosureBehavior(context, reference, current);
			result.add(mapped);
		}
		return result;
	}
	...
	public NumberType sum(@SuppressWarnings("unused") ExecutionContext context, ElementReferenceType reference) {
		NumberType sum = null;
		for (BasicType current : backEnd) {
			NumberType mapped = (NumberType) runClosureBehavior(context, reference, current);
	...
	public GroupingType groupBy(@SuppressWarnings("unused") ExecutionContext context, ElementReferenceType reference) {
		...
		Parameter closureReturnParameter = ActivityUtils.getClosureReturnParameter((Activity) reference.getElement());
		for (BasicType current : backEnd) {
			BasicType groupKey = (BasicType) runClosureBehavior(context, reference, current);
			CollectionType group = groups.get(groupKey);
			if (group == null)
				groups.put(groupKey, group = createCollection(closureReturnParameter.getType(), isUnique(), isOrdered()));
			group.add(current);
		}
		return new GroupingType(baseType, groups);
	}

	public CollectionType collectMany(@SuppressWarnings("unused") ExecutionContext context,
			ElementReferenceType reference) {
		Parameter closureReturnParameter = ActivityUtils.getClosureReturnParameter((Activity) reference.getElement());
		CollectionType result = createCollection(closureReturnParameter.getType(), isUnique(), isOrdered());
		for (BasicType current : backEnd) {
			CollectionType mapped = (CollectionType) runClosureBehavior(context, reference, current);
