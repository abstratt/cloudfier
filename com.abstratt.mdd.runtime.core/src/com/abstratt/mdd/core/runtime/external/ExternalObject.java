...
/**
 * There is only one thing backing all external objects mapping to the same class.
 * 
 * Different classes will have different behaviors.
 * 
 * There is only one delegate taking care of all external behaviors.
 * 
 * The external object makes an external service a 1st class citizen and provides
 * context of the classifier it corresponds to. Not as important when running operations
 * (you could be ok with inferring the classifier from the operation, but critical for signals).
 */
