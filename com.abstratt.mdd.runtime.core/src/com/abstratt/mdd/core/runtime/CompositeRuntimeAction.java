...
public abstract class CompositeRuntimeAction extends RuntimeAction {
	...
	/**
	 * Triggers the creation of runtime object flows for this action and
	 * any actions found under it.
	 * <p>
	 * The composite runtime action provides a scope where runtime actions
	 * are created.
	 * </p>
	 *  XXX an undesirable effect is that currently no sub actions may have flows to/from actions in different scopes  
	 *    
	 * @param nodes
	 */
	...
	public void reset(boolean propagate) {
		...
		for (Iterator i = this.subActions.values().iterator(); i.hasNext();) {
			RuntimeAction current = (RuntimeAction) i.next();
