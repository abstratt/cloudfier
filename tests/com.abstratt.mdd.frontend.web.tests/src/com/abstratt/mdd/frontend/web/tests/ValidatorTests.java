...
public class ValidatorTests extends AbstractWebFrontEndTest {
    private static final URI VALIDATOR_URI = URI.create("http://localhost" + WebFrontEnd.VALIDATOR_PATH
            + "?mdd.enableExtensions=true&mdd.enableLibraries=true");
    ...
    /**
     * @see 0000131: unit contents must be encoded/decoded when
    ...
    public void testEncoding() throws HttpException, IOException {
        ...
        source += "end;\n";
        source += "end.\n";
        HttpMethod method = buildUploadRequest(ValidatorTests.VALIDATOR_URI, Collections.singletonMap("foo.tuml", source.getBytes("UTF-8")));
        HttpClient client = new HttpClient();
        int result = client.executeMethod(method);
        String responseText = method.getResponseBodyAsString();
        TestCase.assertEquals(responseText, 200, result);
        TestCase.assertTrue(responseText, responseText.startsWith("<results status=\"success\""));
    }
    ...
    public void testValidMultipleUnits() throws HttpException, IOException {
        ...
        HttpMethod method = buildUploadRequest(ValidatorTests.VALIDATOR_URI, sources);
        HttpClient client = new HttpClient();
        int result = client.executeMethod(method);
        String responseText = method.getResponseBodyAsString();
        TestCase.assertEquals(responseText, 200, result);
