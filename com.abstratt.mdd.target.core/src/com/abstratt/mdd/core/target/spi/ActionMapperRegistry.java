    ...
    private Map<String, List<IActionMapper>> buildActionMapperRegistry(String extensionPointId) {
        ...
        for (IExtension extension : extensions)
            for (IConfigurationElement configElement : extension.getConfigurationElements()) {
                String action = configElement.getAttribute(ActionMapperRegistry.ATTRIBUTE_ACTION);
                IActionMapper mapper = null;
                try {
                    mapper = (IActionMapper) configElement.createExecutableExtension(ActionMapperRegistry.ATTRIBUTE_MAPPER);
                } catch (CoreException e) {
                    IStatus extensionError = new Status(IStatus.ERROR, TargetCore.PLUGIN_ID, "Error loading extension for "
                            + extensionPointId, e);
