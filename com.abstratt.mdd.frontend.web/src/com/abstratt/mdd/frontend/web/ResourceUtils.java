...
public class ResourceUtils {
	...
	public static Representation serveInResource(Request request, ResourceRunnable runnable) {
		...
		try {
			return RepositoryService.DEFAULT.runInRepository(getRepositoryURI(workspace), runnable);
	...
	/**
	 * Runs the given runnable in the repository corresponding to the given request.
	 */
	...
	public static String getWorkspaceFromProjectPath(Request request) {
		return getWorkspaceFromProjectPath(request, true);
	}

	public static URI getRepositoryURI(String workspace) {
		File deployDirectory = BuildDirectoryUtils.getDeployDirectory(workspace);
		boolean projectExists = deployDirectory.isDirectory() && deployDirectory.listFiles() != null && deployDirectory.listFiles().length > 0;
		if (!projectExists) {
	...
	public static void ensure(boolean condition, String message, org.restlet.data.Status status) {
		if (!condition) {
			...
			LogUtils.logWarning(WebFrontEnd.ID,
	...
	public static void fail(Throwable exception, org.restlet.data.Status status) {
    	ResourceException resourceException = new ResourceException(status == null ? org.restlet.data.Status.SERVER_ERROR_INTERNAL : status, exception);
		LogUtils.logError(WebFrontEnd.ID, exception.getMessage(), exception);
		throw resourceException;
	}
	
	public static List<IProblem> compile(File baseDir,
			Map<String, String> customRepositoryProperties)
			throws CoreException {
		IFileSystem localFS = EFS.getLocalFileSystem();
		final IFileStore basePath = localFS.getStore(baseDir.toURI());
		overrideRepositoryProperties(basePath, customRepositoryProperties);
		...
		RepositoryService.DEFAULT.runInRepository(MDDUtil.fromJavaToEMF(baseDir.toURI()), new ISharedContextRunnable<IRepository,Object>() {
			public Object runInContext(IRepository repository) {
				int mode = ICompilationDirector.CLEAN
						| ICompilationDirector.FULL_BUILD;
				...
				try {
					IProblem[] compilationProblems = CompilationDirector.getInstance()
							.compile(null, repository, context, mode, null);
				...
				for (IProblem compilationProblem : allProblems)
					if (compilationProblem.getSeverity() == IProblem.Severity.ERROR)
						return null;
				allProblems.add(generateInfoMessage(startCompilation,
						endCompilation, "Model compiled successfuly"));
				return null;
			}
		});
		return allProblems;
	...
	private static IProblem generateInfoMessage(long start, long end,
			String phase) {
		return new UnclassifiedProblem(Severity.INFO, phase + " in "
				+ ((end - start) / 1000d) + "s");
	}
	 ...
	 */
	private static void overrideRepositoryProperties(final IFileStore basePath,
			Map<String, String> customRepositoryProperties)
			throws CoreException {
		...
		for (Entry<String, String> entry : customRepositoryProperties
				.entrySet()) {
		...
		try {
			properties.store(output, "Generated by "
					+ ResourceUtils.class.getPackage().getName());
			// properties are always ISO-8859-1
			FileUtils.writeStringToFile(new File(basePathAsFile,
					IRepository.MDD_PROPERTIES), output.toString(),
					"ISO-8859-1");
		} catch (IOException e) {
			throw new CoreException(new Status(IStatus.ERROR, WebFrontEnd.ID,
					"Error updating properties", e));
		}
	}

	/**
	 * Encodes problems as a JSON representation (who am I kidding, this is the Orion format). 
	 */
	public static String buildJSONResponse(
        ...
        for (IProblem current : results) {
			...
			problemMarkup.add ("{ \"reason\": \"" + current.getMessage() + "\", \"end\": 1, \"character\": 1, \"line\": " + lineNumber + ", \"severity\": \"" + current.getSeverity().name().toLowerCase() + "\", \"file\": \"" + filename + "\"}");
		}
	...
	public static StringBuffer buildResponse(List<String[]> ignored,
			List<IProblem> results, double elapsedTime) {
		...
		for (IProblem current : results) {
			...
			Integer lineNumber = (Integer) current
					.getAttribute(IProblem.LINE_NUMBER);
			String fileName = localFileName == null ? ""
					: (localFileName instanceof IFileStore ? ((IFileStore) localFileName)
							.getName() : localFileName.toString());
			String lineNumberString = lineNumber == null ? "" : lineNumber
					.toString();
			problemsMarkup.append("\t\t<" + tag + " file=\"" + fileName
					+ "\" line=\"" + lineNumberString + "\" message=\"");
			problemsMarkup.append(StringEscapeUtils.escapeXml(current
					.getMessage().trim()));
		...
		for (String[] current : ignored)
			ignoredMarkup.append("\t\t<location uri=\"" + current[0]
					+ "\" reason=\"" + current[1] + "\"/>\n");
		ignoredMarkup.append("\t</ignored>\n");
		StringBuffer result = new StringBuffer();
		result.append("<results status=\""
				+ (hasErrors ? "failure" : "success") + "\" duration=\""
				+ elapsedTime + "\">\n");
		if (!results.isEmpty())
	...
	public static List<String[]> fetchUnits(File baseDir, String[] allLocations)
		...
		for (String location : allLocations) {
			if (location.endsWith("/") || location.indexOf("/") < 0) {
				ignored.add(new String[] { location,
						"URI must end with a file name" });
				continue;
			}
			String fileName = location.substring(location.lastIndexOf('/') + 1);
			if (fileName.indexOf('.') < 0) {
				ignored.add(new String[] { location,
						"URI must end with a file name that has an extension" });
				continue;
			}
	...
	public static Representation buildErrorResponse(Throwable e) {
		...
		return new StringRepresentation(
				"<results status=\"failure\" message=\"" + e.getMessage()
						+ "\">\n");
	}

	public static Representation createZip(Map<String, byte[]> files,
		...
		FileRepresentation representation = new FileRepresentation(workingFile,
				MediaType.APPLICATION_ZIP);
		representation.setAutoDeleting(true);
		Series<Parameter> parameters = new Series<Parameter>(Parameter.class,
				Arrays.asList(new Parameter("filename", fileName + ".zip")));
	 ...
	 * @param contextFile
	 * @return the temp directory provisioned, it is up to the caller to delete it
	 */
