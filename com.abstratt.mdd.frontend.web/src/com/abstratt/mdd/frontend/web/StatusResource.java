...
public class StatusResource extends ServerResource {
	...
	public Representation getServiceStatus() {
		...
		String availabilityThresholdParam = (String) getQuery().getValues(
				"available");
		String allocationThresholdParam = (String) getQuery().getValues(
				"allocated");
		try {
			if (availabilityThresholdParam != null)
				availabilityThreshold = Double
						.parseDouble(availabilityThresholdParam) / 100;
			if (allocationThresholdParam != null)
				allocationThreshold = Double
						.parseDouble(allocationThresholdParam) / 100;
		...
		if (allocated > allocationThreshold
				&& available < availabilityThreshold) {
			setStatus(Status.SERVER_ERROR_INTERNAL, "allocation: "
		...
		result.append("<p>Max memory: "
				+ memoryFmt.format(maxMemory / megaByte) + "</p>");
		result.append("<p>Total memory: "
				+ memoryFmt.format(totalMemory / megaByte) + "</p>");
		result.append("<p>Free memory: "
				+ memoryFmt.format(freeMemory / megaByte) + "</p>");
