...
public class PublisherResource extends AbstractWorkspaceResource {
	...
	public Representation getWorkspaceInfo() throws IOException {
		...
		if (!secret.isEmpty()) {
			baseReference.setQuery(SECRET_PARAMETER + "=" + getSecret());
		}
		for (File file : workspaceFiles) {
			...
			String fileURI = baseReference.clone().addSegment(file.getName())
					.toString();
			...
			fileElements.add("<" + element + " name='" + file.getName()
					+ "' uri='" + fileURI + "'" + additionalAttributes + "/>");
		}
		Collections.sort(fileElements);
		StringBuffer workspaceMarkup = new StringBuffer(
				"<workspace packageCount='").append(packageCount)
				.append("' timestamp='").append(timestamp).append("'>\n");
		for (String element : fileElements)
			workspaceMarkup.append("\t").append(element).append('\n');
		Properties repoProperties = MDDUtil.loadRepositoryProperties(MDDUtil
				.fromJavaToEMF(getWorkspaceDir(false).toURI()));
...
//					+ "\" uri=\"" + generatorURI + "\"/>\n");
//		}
		String archiveURI = baseReference.getParentRef()
				.addSegment(getWorkspaceDir(false).getName() + ".zip").toString();
	...
	private String getClasses(File workspaceDir) throws CoreException {
		...
		IRepository repository = MDDCore.createRepository(URI
				.createURI(workspaceDir.toURI().toString()));
		try {
			final List<Classifier> userClasses = repository.findAll(
					new EObjectCondition() {
						@Override
						public boolean isSatisfied(EObject eObject) {
							if (!UMLPackage.Literals.CLASSIFIER
									.isInstance(eObject))
								return false;
							if (UMLPackage.Literals.ACTIVITY
									.isInstance(eObject))
								return false;
							if (UMLPackage.Literals.ASSOCIATION
									.isInstance(eObject))
								return false;
							Classifier asClassifier = (Classifier) eObject;
							return !StringUtils.isBlank(asClassifier
									.getQualifiedName())
									&& !TemplateUtils
											.isTemplateInstance(asClassifier);
						}
					}, true);
			for (Classifier each : userClasses) {
				...
				if (unitAnnotation != null)
					for (Entry<String, String> entry : unitAnnotation
							.getDetails().entrySet())
						response.append("unit." + entry.getKey() + "=\""
								+ entry.getValue() + "\" ");
				response.append("/>\n");
			}
		} finally {
	...
	public Representation buildWorkspace(Representation entity) {
		...
		try {
			...
			String secret = getSecret();
			if (secret != null) {
				File secretFile = new File(tmpProjectDir, SECRET_FILE);
	...
	protected Map<String, String> getMDDOptions() {
		...
		for (Iterator<Entry<String, String>> iterator = allEntries.iterator(); iterator
				.hasNext();)
			if (!((Entry<String, String>) iterator.next()).getKey().startsWith(
					"mdd."))
				iterator.remove();
